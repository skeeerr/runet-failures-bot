# Оптимизированная версия Telegram-бота на основе двух представленных вариантов
...
COMMANDS_TEXT = (
    ...
    "/stats - статистика пользователей (для админов)"
)
...
async def handle_start(message: types.Message):
    ...
    if ref_id and ref_id != message.from_user.id:
        await send_admin_log(f"👤 Новый пользователь: {full_name} зарегистрировался по рефералке от ID {ref_id}")
...
async def show_app_stats(callback: types.CallbackQuery):
    ...
    try:
        ...
        text = f"⚠️Информация о работе {name.capitalize()}\n\n📊 График жалоб за последние часы\n\n🛜@nosignalrubot"
...
async def menu_refstats(callback: types.CallbackQuery):
    ...
    if not top_users:
        await bot.send_message(callback.from_user.id, "Реферальная статистика пока пуста.")
        return
    lines = []
    ...
    async def handle_broadcast_text(msg: types.Message):
        ...
        for user in users:
            try:
                ...
                failed += 1
                await send_admin_log(f"Ошибка при рассылке {user['user_id']}: {e}")
